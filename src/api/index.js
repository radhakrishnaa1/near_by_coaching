...
//   let sql = "SELECT (SELECT COUNT(*) FROM tutor_enquiry Where tutor_id = ?) AS total , (SELECT COUNT(*) FROM tutor_enquiry where status = 'paid' AND tutor_id = ?) AS totalPaid, (SELECT COUNT(*) FROM tutor_enquiry WHERE status = 'fee' AND tutor_id = ?) AS feetopay , (SELECT COUNT(*) FROM tutor_enquiry Where status = 'Enquiry' AND tutor_id = ?) AS totalenquiry";
...
//     institute_id,
//     creation_date} = request.body;
//   let sql = "INSERT INTO course_details ( course_name, course_duraton, course_fee, status, mode, timing, course_medium, start_date, end_date, discount,max_student,course_details,institute_id, creation_date) VALUES (?,?, ?, ?, ?,?,?,?,?,?,?,?,?,?)";   
//   connection.query(sql, [
//     course_name, 
//     course_duraton, 
//     course_fee, 
